...
#include "Utilities/Log.h" // Explicitly include Log.h
...
FSM_StateName FSM_State_Locomotion<T>::checkTransition() {
  ...
  if(locomotionSafe()) {
    ...
    this->nextStateName = FSM_StateName::RECOVERY_STAND;
    this->transitionDuration = 0.;
    rc_control.mode = RC_mode::RECOVERY_STAND; // Update RC control mode
    this->_data->controlParameters->control_mode = K_RECOVERY_STAND; // Force global control mode to K_RECOVERY_STAND
  }
...
void FSM_State_Locomotion<T>::LocomotionControlStep() {
  ...
  // [DEBUG LOG] 仿照 Balance Stand 添加的调试信息
  static int s_loco_log_decimator = 0;
  if ((s_loco_log_decimator++ % 100) == 0) { // 每100次循环打印一次
    auto& se = this->_data->_stateEstimator->getResult();
    // 打印当前实际位置 vs MPC期望位置
    LOG_INFO("[LOCO] BodyPos Est=[{:.3f} {:.3f} {:.3f}] Des=[{:.3f} {:.3f} {:.3f}]",
             (double)se.position[0], (double)se.position[1], (double)se.position[2],
             (double)cMPCOld->pBody_des[0], (double)cMPCOld->pBody_des[1], (double)cMPCOld->pBody_des[2]);
    
    // 打印当前实际姿态 vs MPC期望姿态
    LOG_INFO("[LOCO] BodyRPY Est=[{:.3f} {:.3f} {:.3f}] Des=[{:.3f} {:.3f} {:.3f}]",
             (double)se.rpy[0], (double)se.rpy[1], (double)se.rpy[2],
             (double)cMPCOld->pBody_RPY_des[0], (double)cMPCOld->pBody_RPY_des[1], (double)cMPCOld->pBody_RPY_des[2]);

    // 打印第一条腿的受力情况 (MPC计算出的力)
    auto& fr = cMPCOld->Fr_des[0];
    LOG_INFO("[LOCO] Leg0 Fr_MPC=[{:.1f} {:.1f} {:.1f}] Contact={}", 
             (double)fr[0], (double)fr[1], (double)fr[2], 
             (int)cMPCOld->contact_state[0]);
  }
